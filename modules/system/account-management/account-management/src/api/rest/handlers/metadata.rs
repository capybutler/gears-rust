...
/// `resource_name` carries the chained `schema_id` from the URL.
///
/// # Errors
///
/// Surfaces a canonical `Problem` envelope. Notable codes:
/// `metadata_validation` (400 -- malformed chained `schema_id`),
/// `cross_tenant_denied` (403), `not_found` (404 -- unified
/// metadata 404), `service_unavailable` (503).
#[tracing::instrument(
    skip(svc, ctx),
    fields(tenant_id = %tenant_id, schema_id = %schema_id_raw, request_id = Empty)
)]
pub async fn get_metadata(
    Extension(ctx): Extension<SecurityContext>,
    Extension(svc): Extension<Arc<MetadataService>>,
    Path((tenant_id, schema_id_raw)): Path<(Uuid, String)>,
) -> ApiResult<Json<TenantMetadataEntryDto>> {
    let schema_id = GtsSchemaId::new(&schema_id_raw);
    let entry = svc.get_metadata(&ctx, tenant_id, schema_id).await?;
...
///
/// Surfaces a canonical `Problem` envelope. Notable codes:
/// `metadata_validation` (400 -- malformed `schema_id`, null `value`,
...
pub async fn upsert_metadata(
    ...
    let schema_id = GtsSchemaId::new(&schema_id_raw);
    let request = UpsertMetadataRequest::new(schema_id, req_body.value);
...
///
/// Surfaces a canonical `Problem` envelope. Notable codes:
/// `metadata_validation` (400 -- malformed `schema_id`),
...
pub async fn delete_metadata(
    ...
    let schema_id = GtsSchemaId::new(&schema_id_raw);
    svc.delete_metadata(&ctx, tenant_id, schema_id).await?;
...
pub async fn resolve_metadata(
    ...
    let schema_id = GtsSchemaId::new(&schema_id_raw);
    let echoed = schema_id.as_ref().to_owned();
    let resolution = svc.resolve_metadata(&ctx, tenant_id, schema_id).await?;
