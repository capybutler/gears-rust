...
class TimescaleDbSidecar:
    ...
    def start(self) -> None:
        ...
        # Pull separately, with its own generous timeout: `docker run` below
        # shares `_run`'s DOCKER_TIMEOUT (120s), which is fine for starting an
        # already-local image but too tight if it also has to pull first. A
        # `subprocess.TimeoutExpired` during `docker run` would escape as a
        # bare SubprocessError instead of this module's RuntimeError, and
        # since `atexit.register(self.stop)` hasn't run yet, the container
        # (if the daemon managed to create it before the timeout) would leak
        # until the next reap. Pulling first keeps `docker run` itself fast
        # and idempotent when the image is already present.
        _run(["docker", "pull", self.IMAGE], timeout=600)
