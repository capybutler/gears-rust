...
async def test_list_records_odata_filter_and_cursor(api, make_usage_type):
    ...
    `limit` is the only wire page-size parameter: it is what the OpenAPI
    contract documents and what the toolkit's OData extractor
    (`ODataParams` in libs/toolkit/src/api/odata.rs) parses. Canonical
    OData `$top` is rejected as an unrecognized parameter, because nothing
    would bind it to `ODataQuery.limit`.
    """
    gts_id, _ = await make_usage_type()
    async with api() as client:
