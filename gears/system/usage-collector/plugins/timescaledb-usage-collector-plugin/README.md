...
The crate implements `usage_collector_sdk::UsageCollectorPluginV1` (via `StorageAdapter` over the record store). Conformance is enforced at compile time: the adapter satisfies the trait, so a drift between the SPI and this backend is a build error.
