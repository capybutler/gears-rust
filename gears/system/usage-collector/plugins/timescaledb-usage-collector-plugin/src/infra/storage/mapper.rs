...
pub fn metadata_jsonb_to_map(
    ...
    for (key, val) in obj {
        ...
        let metadata_key = MetadataKey::new(key.clone()).map_err(|e| {
            UsageCollectorPluginError::internal(format!("stored metadata key `{key}` invalid: {e}"))
        })?;
...
pub fn type_row_to_model(row: UsageTypeRow) -> Result<UsageType, UsageCollectorPluginError> {
    ...
    for field in row.metadata_fields {
        let key = MetadataKey::new(field.clone()).map_err(|e| {
            UsageCollectorPluginError::internal(format!(
                "stored metadata_fields entry `{field}` invalid: {e}"
            ))
        })?;
