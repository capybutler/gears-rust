...
//! all-ascending `(created_at, uuid)` tuple, so [`keyset_predicate`] emits the
...
//!   `to_signed_tokens() -> String` (`"+created_at,+uuid"`) and
...
/// Render an `ORDER BY` column list (`"created_at ASC, uuid ASC"`) from an
...
/// `(created_at, uuid)` ascending, so the tuple form covers the live path;
...
/// `cursor_keys`, a field is not on the allowlist, a field has no known kind,
/// the directions are mixed, or a cursor key cannot be parsed.
...
///
/// `s` carries the signed sort tokens (`"+created_at,+uuid"`); `o` is the
