...
fn record_field_columns_are_allowlisted() {
    assert_eq!(record_column("uuid"), Some("uuid"));
...
fn order_created_at_uuid() -> ODataOrderBy {
    ODataOrderBy(vec![
        OrderKey {
...
fn render_order_by_renders_allowlisted_columns() {
    let sql = render_order_by(&order_created_at_uuid(), record_column).unwrap();
    assert_eq!(sql, "created_at ASC, uuid ASC");
}
...
fn keyset_predicate_ascending_renders_tuple_comparison_with_two_binds() {
    let pairs: &[(&str, bool)] = &[("created_at", true), ("uuid", true)];
    let keys = vec![
        "2026-01-02T03:04:05Z".to_owned(),
        uuid::Uuid::from_u128(0x1234).to_string(),
    ];
    let mut ctx = SqlCtx::new(1);
    let sql = keyset_predicate(pairs, &keys, record_column, rec_kind, &mut ctx).unwrap();
    assert_eq!(sql, "(created_at, uuid) > ($1, $2)");
    assert_eq!(ctx.binds.len(), 2);
...
fn keyset_predicate_descending_uses_less_than() {
    let pairs: &[(&str, bool)] = &[("created_at", false), ("uuid", false)];
    let keys = vec![
        "2026-01-02T03:04:05Z".to_owned(),
        uuid::Uuid::from_u128(0x1234).to_string(),
    ];
...
fn keyset_predicate_rejects_mixed_directions() {
    let pairs: &[(&str, bool)] = &[("created_at", true), ("uuid", false)];
    let keys = vec!["2026-01-02T03:04:05Z".to_owned(), "x".to_owned()];
    let mut ctx = SqlCtx::new(1);
    assert!(keyset_predicate(pairs, &keys, record_column, rec_kind, &mut ctx).is_err());
}

#[test]
fn keyset_predicate_binds_uuid_column_as_uuid_not_text() {
    // `tenant_id` is a uuid column whose name is neither `uuid` nor
...
fn encode_then_decode_cursor_round_trips_keys_and_order() {
    let order = order_created_at_uuid();
    let keys = vec![
        "2026-01-02T03:04:05Z".to_owned(),
        uuid::Uuid::from_u128(0x1234).to_string(),
    ];
    ...
    assert_eq!(decoded.s, "+created_at,+uuid");
