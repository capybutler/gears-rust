...
pub fn classify_db(code: &str, constraint: Option<&str>) -> DbErrorClass {
    match code {
        ...
        // records PK `(uuid, created_at)` collision) must fall through to
...
/// Whether a `pool.acquire()` failure should clear the `uc.timescaledb.ready`
...
/// outage). This stops the gauge flapping under load (the false-positive the
/// review flagged) without losing connection-refused outage detection (the most
