...
impl PgCatalogStore {
    ...
    ///
    /// Best-effort: a failed count leaves the previous gauge value in place and
    /// is logged at `warn` rather than surfaced (the gauge is observability, not
    /// a contract). The count is read as a `bigint`/`i64` and converted to `u64`
    /// via `try_from` (no `as` cast), flooring a negative (impossible) value at
    /// `0`.
        ...
        match result {
            ...
            Err(err) => {
                        ...
                        == DbErrorClass::ForeignKeyViolation
                {
                    ...
                    // usable, so reuse it for the reference count.
                    let count: i64 =
                        sqlx::query_scalar("SELECT count(*) FROM usage_records WHERE gts_id = $1")
                            .bind(gts_id_str(&gts_id))
                            .fetch_one(&mut *conn)
                            .await
