...
/// `Metrics::new` must construct the full instrument inventory without panicking
/// against the process-global provider.
///
/// These imperative recordings are no-ops without an installed reader, but must
/// not panic; the value assertions live in
/// [`recording_helpers_emit_expected_series`], which installs an in-memory
/// reader via the [`Metrics::with_meter`] seam.
#[tokio::test]
async fn new_constructs_without_panic() {
    let metrics = Metrics::new(lazy_pool());

    metrics.set_ready(true);
    metrics.set_catalog_size(0);
...
async fn recording_helpers_emit_expected_series() {
    ...
    // Counter (labelled): backend errors split by `class`.
    metrics.inc_backend_error(ErrorClass::Transient);
    metrics.inc_backend_error(ErrorClass::Transient);
    ...
    assert_eq!(counter_sum(&exporter, "uc.timescaledb.dedup.absorbed"), 3,);
    assert_eq!(counter_sum(&exporter, "uc.timescaledb.backend.error"), 3,);
    assert_eq!(
        counter_sum_with_label(
            &exporter,
            "uc.timescaledb.backend.error",
            label::CLASS,
            label::CLASS_TRANSIENT,
        ),
        2,
    );
    assert_eq!(
        gauge_last_u64(&exporter, "uc.timescaledb.usage_type_catalog.size"),
        Some(42),
    );
    assert_eq!(gauge_last_u64(&exporter, "uc.timescaledb.ready"), Some(1));
    assert_eq!(
        histogram_count(&exporter, "uc.timescaledb.insert.duration"),
        2,
    );
}
