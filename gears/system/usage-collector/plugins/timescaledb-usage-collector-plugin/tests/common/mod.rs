...
/// Monotonic seed so each raw record insert gets a distinct `uuid` and
...
pub async fn bring_up() -> anyhow::Result<TsHarness> {
    ...
    let cfg = TimescaleDbPluginConfig {
        ...
        // sslmode are upgraded to `require` — see `connect_options`).
        database_url: format!("postgres://user:pass@127.0.0.1:{port}/app?sslmode=disable"),
        ..Default::default()
    };
...
pub fn record_store(pool: &PgPool) -> PgRecordStore {
    PgRecordStore::new(pool.clone(), metrics(pool))
}
...
///
/// `uuid` is minted from `seq` via [`Uuid::from_u128`] (the workspace `uuid`
...
/// `metadata`, and `ingested_at` take their column defaults. The `uuid` and
...
pub async fn insert_raw_usage_record(
    ...
    let uuid = Uuid::from_u128(u128::from(seq));
    let idempotency_key = format!("raw-idem-{seq}");

    sqlx::query(
        "INSERT INTO usage_records \
         (uuid, tenant_id, gts_id, value, created_at, resource_id, resource_type, idempotency_key) \
