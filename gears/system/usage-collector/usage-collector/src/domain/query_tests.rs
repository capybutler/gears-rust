...
#[test]
fn compose_unconstrained_scope_passes_filter_and_hash_through() {
    let user = user_query_with_gateway_hash(
        "created_at ge 2026-01-01T00:00:00Z and created_at lt 2026-02-01T00:00:00Z",
    );
    let composed = compose_query_with_scope(&user, &AccessScope::allow_all()).expect("compose ok");
    assert_eq!(
        composed.filter_hash, user.filter_hash,
        "allow_all leaves filter_hash unchanged",
    );
    assert_eq!(
        toolkit_odata::short_filter_hash(composed.filter()),
        toolkit_odata::short_filter_hash(user.filter()),
        "allow_all adds no predicate, so the filter is unchanged",
    );
}
