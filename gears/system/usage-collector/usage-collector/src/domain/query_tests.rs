...
fn reserved_field_match_is_case_insensitive() {
    // Matches `require_bounded_time_window`'s own `eq_ignore_ascii_case`
    // precedent for the identical kind of AST-identifier comparison. Not
    // exploitable today (`window_start` / `window_end` are not yet
