...
| Idempotency Key        | The identifier that makes at-least-once processing safe: an exact-equality re-submission under the same key is silently absorbed (no duplicate **Usage Record**), while a same-key submission whose content differs is surfaced as a conflict against a converged **Usage Record** rather than silently dropped. The caller supplies it on a **Usage Record**; the system derives it from the target on an **Invalidation**. Both outcomes are bounded by the **idempotency horizon**, and how the conflict holds under concurrent submissions is the storage plugin's declared **dedup level** (`cpt-cf-usage-collector-fr-idempotency`).                                                                |
| Operational Replay Horizon | How far back a charging consumer may rewind its feed cursor and still be **guaranteed** service — the maximum age of usage it can be required to re-read and re-process. It is the guaranteed window rather than the refusal boundary. Beyond it service is plugin-dependent, and a cursor older than the retention floor is refused (`cpt-cf-usage-collector-fr-billing-retention-floor`). A deployment parameter, 35 days at launch, and one of the two terms of the retention floor. Distinct from the recovery *speed* bounded by `cpt-cf-usage-collector-nfr-replay-throughput`.                     |
| Idempotency Horizon    | How long the gear guarantees duplicate suppression for a given **Usage Record**: at least the referenced GTS type's retention policy, measured from the covered period. It is never reached from the ingestion side: the backfill window bounds an admissible covered period strictly inside it, so a submission old enough to have outlived its counterpart is refused on that bound before deduplication is consulted (`cpt-cf-usage-collector-fr-idempotency`).                                                             |
...
- A replay-safe, pull-based usage feed with per-GTS-type subscription and snapshot-consistent cursors
...
- **GTS type lifecycle**: registration, amendment, and withdrawal of GTS type declarations are owned by the platform `types-registry` gear ([§5.7](#57-usage-record-typing)). The Usage Collector resolves declarations and validates against them; it exposes no GTS type surface of its own and holds no catalog of its own
...
- **Import or correction beyond the backfill window**: the configured window (default 90 days) is a hard bound on every surface. v1 offers no elevated-authorization override reaching past it, so usage older than the window can be neither imported nor withdrawn ([§5.9](#59-billing-integration), [§13](#13-open-questions))
...
The system **MUST** require a client-provided idempotency key on every **Usage Record**. The system **MUST** reject any **Usage Record** submitted without an idempotency key with an actionable error. The dedup identity of a submission is its idempotency key together with the tenant, the GTS type, and the window bounds (see `cpt-cf-usage-collector-fr-usage-windows` and `cpt-cf-usage-collector-fr-record-identity`); submissions that differ in any part of that identity are distinct **Usage Records** rather than duplicates or conflicts. The same dedup identity governs an invalidation entry, whose window bounds are those of the **Usage Record** it withdraws ([§5.6](#56-corrections)), but its key is not client-provided: the system **MUST** derive it as `inv:` followed by the identifier of that **Usage Record**, and **MUST** reject an invalidation entry that supplies a key of its own with an actionable error. When a submission matches a converged **Usage Record** (see **Concurrent submissions** below) on the dedup identity **and** every caller-supplied field is identical, the system **MUST** silently deduplicate the submission (no error, no duplicate **Usage Record**); this is the exact-equality retry case. When it matches that **Usage Record** on the dedup identity but **any** caller-supplied field differs — including a metadata-only difference — the system **MUST** reject the submission with an actionable conflict error and **MUST NOT** silently drop the second write. Two submissions under one dedup identity inside one ingestion request resolve the same way, the later against the earlier.

**Concurrent submissions.** Two submissions under one dedup identity can reach the store before either has seen the other. Every storage plugin **MUST** decide them by the store's own *commit order* — the order in which it makes submissions under one dedup identity durable, as the plugin's own state records it, never a gateway timestamp. The first submission in commit order is the *survivor*. Every read path, every aggregation, reconciliation metadata (`cpt-cf-usage-collector-fr-reconciliation-metadata`), and the usage feed **MUST** show the survivor and no other **Usage Record** under that identity; a read before convergence may show a submission that proves not to be first, never two. A dedup identity has *converged* once its survivor is final — no submission preceding it in commit order can still become visible — the survivor is visible to every dedup check the storage plugin runs, and no persist call under the identity that did not see the survivor is still to return its outcome; the survivor is then a converged **Usage Record**, until retention frees the identity no other submission under it ever is, and every later submission is decided against it. A plugin **MUST** establish convergence from its own commit or replication state rather than from elapsed time, and **MUST** publish a *convergence bound*, the longest time from an acknowledgement to convergence. No later submission displaces a converged survivor. The guarantee is tied to the moment the storage plugin returns its outcome, not to when a submission arrived, and gateway forwarding after that moment does not count: no acknowledgement returned after the identity has converged accepts divergent content under it — an identical submission is absorbed and a divergent one is rejected with the conflict error — while an acknowledgement returned before convergence may be of a submission that is later discarded. A submission whose caller has already been answered — such as a timed-out insert that commits late — is discarded and, when it diverges, counted. Two submissions under one dedup identity inside one ingestion request are decided by the system before dispatch, the later against the earlier accepted one. Each plugin **MUST** also declare, as part of its published consistency profile (`cpt-cf-usage-collector-nfr-query-freshness`), one of two **dedup levels**:

- **`linearizable`** — the convergence bound is zero: every submission is decided against every earlier one in commit order as it commits, an identical one absorbed and a divergent one rejected with the conflict error.
- **`eventual`** — a submission may be acknowledged before the plugin knows whether an earlier submission precedes it. When one does, the later submission **MUST** be discarded once the identity converges, and each divergent discard **MUST** be counted on an operator-visible metric. The usage feed **MUST NOT** return a discarded submission. The acknowledgement of a discarded submission can disagree with later reads — in acceptance instant and origin marker, and for a divergent submission in content — and a later retry of a discarded divergent submission draws the conflict error.

An `eventual` plugin is the one case in which the system drops a divergent write without a conflict error. It arises only from an emitter defect — one key reused with different content and acknowledged before the identity has converged — since a genuine retry repeats its content and loses nothing when absorbed. The level therefore gates no meter, charging meters included.

**Key construction.** The key **MUST** distinguish what the dedup identity does not: two resources or subjects measured over one period, and two **Usage Records** covering one period. A retry **MUST** repeat its submission's key exactly, since a retry under a fresh key is a new **Usage Record** rather than a duplicate. The `inv:` prefix is reserved for derived invalidation keys: the system **MUST** reject a **Usage Record** whose key begins with it, with an actionable error, so an invalidation entry is distinct from its target by construction ([§5.6](#56-corrections)).
...
**No admissible submission outlives the horizon.** The covered period is both what the dedup identity carries and what the ingestion bounds check, and the retention floor keeps the backfill window strictly inside retention (`cpt-cf-usage-collector-fr-billing-retention-floor`). A submission old enough for its counterpart to have been purged is therefore refused on the window bound before deduplication is consulted, and no caller carries a post-horizon obligation. A charging consumer still deduplicates on **Usage Record** identifier, for the at-least-once feed delivery that is its own reason (`cpt-cf-usage-collector-fr-billing-usage-feed`).

Three paths approach the boundary, and none of them crosses it:
...
| `COUNT`  | not read — the fold counts **Usage Records**, and a withdrawn pair counts none | not applicable              |
...
| `LATEST` | one observation; the fold reports the one with the greatest covered-period end in range, ties broken by the later acceptance instant, then the greater identifier in byte order | no                               |

**The quantity under `COUNT`.** A `COUNT` meter's records still carry a quantity, because the record shape is the same for every GTS type, but it means nothing: under `COUNT` one record is one event, and the fold reads only how many there are. An emitter **SHOULD** send `1`, and a consumer **MUST NOT** read a `COUNT` meter's quantity as a measurement. Ingestion does not enforce the convention — it never consults the declared fold, so a `COUNT` meter's record is accepted or rejected on exactly the terms any other record is (`cpt-cf-usage-collector-fr-record-quantity`).

**What `COUNT` counts.** The fold **MUST** count the **Usage Records** in range that no accepted invalidation entry withdraws, and **MUST NOT** count an invalidation entry or the record it withdraws. This is the withdrawal exclusion read under `COUNT` (`cpt-cf-usage-collector-fr-record-invalidation`): a withdrawn record counts 1 and its invalidation counts 1, so a fold admitting both returns 2 where the answer is 0.

**Ties under `LATEST`.** Two **Usage Records** of one meter can share a covered period, since they are distinguished by idempotency key as well as period (`cpt-cf-usage-collector-fr-record-identity`), and `LATEST` selects a record rather than reducing values — so an undefined tie would let two consumers read the same range and obtain different quantities. The order is therefore total: the greatest covered-period end, then the later acceptance instant (`cpt-cf-usage-collector-fr-usage-windows`), then the greater **Usage Record** identifier in byte order, which is unique (`cpt-cf-usage-collector-fr-record-identity`). The gear assigns every key and each compares across tenants, so the order stays total when an aggregation group spans tenants — as it does for a request that neither narrows to one tenant nor groups by tenant. `MAX` and `MIN` need no such rule: a tie there returns the same value whichever record supplied it.
...
The system **MUST** provide an API for querying aggregated usage data. Queries **MUST** support time-bounded aggregation for exactly one GTS type. The time range is mandatory and narrows every aggregation. Queries **MUST** allow consumers to narrow and group results by tenant, subject, and resource where authorized.
...
**One value per group.** The time range selects the entries an aggregation folds. It does not divide the result into sub-periods, and a time bucket is not a grouping dimension in v1. A consumer that needs a per-period series requests one aggregation per period, or reads the raw query path (`cpt-cf-usage-collector-fr-query-raw`) and folds the entries itself.
...
**Stability after the backfill horizon.** No entry whose covered period ends further back than the configured backfill window is accepted on either ingestion path, and an invalidation carries the period of the **Usage Record** it withdraws. An aggregate over a period entirely older than the backfill window therefore cannot change at all: the window is a hard bound on every surface, and no ingestion path admits an entry that would alter such an aggregate (`cpt-cf-usage-collector-fr-backfill`). A consumer that closes its books on that horizon can rely on the figure it read.
...
**Invalidated records.** Raw query is a ledger read path, not a derived view: a **Usage Record** withdrawn by an accepted invalidation entry, and the invalidation entry itself, **MUST** both be returned as persisted, the invalidation naming the record it withdraws (`cpt-cf-usage-collector-fr-billing-fields-on-read`). The path applies no fold, so it has none to correct; leaving a withdrawn pair out of a fold computed from these rows is the reader's obligation (`cpt-cf-usage-collector-fr-aggregation-fold`), and the aggregate path is where the gear discharges it (`cpt-cf-usage-collector-fr-query-aggregation`).
...
**What an invalidation entry carries.** An invalidation entry travels the ingestion path a measurement of its covered period would travel, and is a **faithful copy** of the **Usage Record** it withdraws: every caller-supplied field — tenant, GTS type, resource, subject, covered period, quantity, and metadata — **MUST** equal the corresponding field of the target and is validated against it at ingestion. The departures from that copy are closed, and are exactly three: the entry **MUST** name the target by reference, **MUST** carry a reason code (`cpt-cf-usage-collector-fr-invalidation-reason-code`), and carries an idempotency key the system derives from that reference rather than one the caller supplies (`cpt-cf-usage-collector-fr-idempotency`). Naming the target is itself what marks the entry as an invalidation, so there is no separate marker a caller could set inconsistently with the reference it accompanies. The gear assigns it four server-side fields in its own right rather than by copy. Three it assigns to any accepted entry: an identifier (`cpt-cf-usage-collector-fr-record-identity`), an acceptance instant (`cpt-cf-usage-collector-fr-usage-windows`), and the origin marker of the path it arrived on (`cpt-cf-usage-collector-fr-backfill`). The fourth is its derived idempotency key. Being accepted in its own right gives the invalidation its own position on the feed, which the feed's correction order places after the **Usage Record** it withdraws (`cpt-cf-usage-collector-fr-billing-usage-feed`).
...
- **Valid reference**: an invalidation entry **MUST** name an existing **Usage Record** whose dedup identity has converged, by that record's identifier (`cpt-cf-usage-collector-fr-record-identity`, `cpt-cf-usage-collector-fr-idempotency`). A reference resolving to nothing is rejected with an actionable error. A target that has not yet converged is rejected with a retryable error, so the copy is never validated against a submission the storage plugin later discards. The lookup **MUST NOT** report an acknowledged, retained **Usage Record** as resolving to nothing, and **MUST** reach a definite answer — the target, or nothing — within the storage plugin's convergence bound plus its published query-path lag bound, so a reference to an identifier that never existed is refused rather than retried indefinitely. A target outside the caller's authorized scope **MUST** draw exactly the answers an absent one would.
...
- **At most one per record**: a **Usage Record** **MUST** be withdrawn at most once. Because the system derives an invalidation entry's key from its target, every invalidation of one **Usage Record** shares one dedup identity: a second one carrying the same reason code is absorbed as a duplicate of the first, and one carrying a different reason code **MUST** be rejected with an actionable error stating that the target is already invalidated. Two such submissions racing before their identity has converged resolve at the storage plugin's declared dedup level (cross-reference `cpt-cf-usage-collector-fr-idempotency`).
- **Path-owned period bounds**: the covered period an invalidation copies **MUST** be validated against the bounds of the path it arrived on, exactly as an ordinary **Usage Record**'s period is (`cpt-cf-usage-collector-fr-live-future-time-bound`, `cpt-cf-usage-collector-fr-backfill`). The entry kind confers no wider retroactive reach. Withdrawing a **Usage Record** older than the live past tolerance therefore belongs on the backfill path, which the live-path rejection **MUST** name, and beyond that path's window it cannot be withdrawn at all, since no ingestion path admits the covered period a faithful copy would have to carry.
...
A **Usage Record**'s GTS type is registered in `types-registry`, which owns its whole lifecycle. The Usage Collector **MUST NOT** expose a GTS type operation of any kind — read or write — on any of its surfaces, and **MUST NOT** maintain a second catalog that has to be kept in step with the registry.
...
Because the period end is exclusive, a **Usage Record** whose period ends exactly on the range's upper bound belongs to the next range. This is the expected shape, not a defect to work around: an emitter tiles its periods edge to edge, and a consumer that wants a given calendar period shifts the requested range forward by one emission period.
...
An invalidation entry is identified by the same derivation over the same four attributes, which is well defined because [§5.6](#56-corrections) fixes the period it carries to that of the **Usage Record** it withdraws. Its identifier differs from its target's because its idempotency key is the one the system derives from the target (`cpt-cf-usage-collector-fr-idempotency`), which no **Usage Record** key can equal. The identifier is therefore reproducible offline from the target alone, and every invalidation of one **Usage Record** carries the same one. Entry type is deliberately **not** part of the derivation: the reserved key prefix already keeps a measurement and its withdrawal apart.
...
- **Past bound.** The system **MUST** reject a **Usage Record** whose covered period **ends** further into the past than the configured past tolerance (default 48 hours). The error **MUST** identify the offending instant, the bound, and the dedicated backfill route as the path such a submission belongs on (`cpt-cf-usage-collector-fr-backfill`).
...
Both bounds read the end of the covered period, so the live path admits a covered period of any length whose end falls inside the past tolerance. A meter that accrues over a month emits there as soon as its period closes, and its entry carries the live origin marker. No bound reads the period start, because no read path selects on it (`cpt-cf-usage-collector-fr-usage-windows`).
...
- **Rationale**: Emitter clock skew must not open a not-yet-existing consumption period, and a bounded future tolerance turns silent time-corruption into a visible emitter defect. The past bound exists so that the origin marker can separate imported history from current consumption. A submission whose consumption ended long ago arrives marked as live, which defeats that marker, so the bound reads the period end. A period start reaching further back does not make the consumption stale. A default of 48 hours covers emitter outage and retry lag, which is what genuinely late live data is. Anything older is history, and history belongs on the route that marks it and isolates its load. Naming that route in the error lets an emitter act on the rejection rather than guess. That gap closes only if the bound governs every entry. A withdrawal of a closed period is a correction of history, so exempting it would re-open the marker gap and put the emitter-defect workload back on the live path.
- **Actors**: `cpt-cf-usage-collector-actor-usage-source`
...
Raw query, point lookup by identifier, and the usage feed **MUST** return the following unstripped: the **Usage Record**'s identifier, the GTS type it references, the period it covers, its acceptance instant, its declared metadata values, its signed quantity, its entry type, the **Usage Record** it withdraws and its reason code where it is an invalidation, and the origin marker that distinguishes a backfilled **Usage Record** (`cpt-cf-usage-collector-fr-backfill`). The metering unit and the aggregation fold are resolved from the GTS type declaration and are not carried per **Usage Record**. Point lookup by identifier **MUST** return the exact persisted fact.

No read path carries a reverse link. Its value would depend on entries accepted later, so on the usage feed it would change an entry already delivered and break the replay guarantee (`cpt-cf-usage-collector-fr-billing-usage-feed`). A reader holding a **Usage Record** finds any withdrawal of it with one raw query, filtering on the target reference over the period that record covers.
...
- **a deterministic order**: the same cursor **MUST** yield the same entries in the same order, extended only by entries accepted since, and a replay bounded by a cursor recorded later **MUST** be identical entry for entry. The storage plugin chooses the order, and no position value is part of the contract. Beyond determinism the system promises one ordering property only: an invalidation entry is delivered after the **Usage Record** it withdraws. In particular, entries are not promised in acceptance-instant order, and a consumer detects late arrival from the acceptance instant rather than from feed order;
- **completeness behind the cursor**: everything before a returned cursor has been delivered and is final. The system **MUST NOT** make any entry visible behind a returned cursor, however many writers accept concurrently, so a consumer resuming from a persisted cursor misses nothing. On a live read every page **MUST** return a next cursor, including a page with fewer entries than requested: the feed has no end, and a later request with that cursor returns only entries that became available since. Only a replay bounded by a cursor recorded later reaches an end, and its last page carries no next cursor;
- stable cursor pagination over a consistent snapshot: a paginated scan **MUST NOT** observe feed entries appearing, disappearing, or changing mid-scan, except append-only arrivals, which always land ahead of the cursor. The cursor is **opaque**, and it is the only token a consumer holds;
- **subscription to a subset of GTS types**, so a consumer reads only the meters it rates. A consumer **MUST** be able to declare the set of GTS types it consumes, and the feed **MUST** exclude everything else, including from the cursor it returns for that subscription;
- correction linkage per `cpt-cf-usage-collector-fr-billing-fields-on-read`, so a reader can reconstruct correction history;
- **corrections as ordinary entries**. Every correction is an appended invalidation entry, so it enters the feed as itself at its own feed position, after the entry it withdraws, and no feed entry represents a change to an already-delivered **Usage Record**. The snapshot rule above therefore holds over **Usage Records** directly: an already-read entry never changes, and a correction to it arrives as a later entry. An accepted invalidation **MUST NOT** remove either entry from the feed — withdrawal is expressed by the arrival of the later entry, never by the disappearance of the earlier one — so a replay from a cursor within the retention floor observes the same entries the original scan observed. A negative quantity likewise arrives as an ordinary entry, since it is a measurement rather than a correction ([§5.6](#56-corrections)).
...
- A cursor older than the retention floor **MUST** be refused with an actionable error.
- Between those two ages service is plugin-dependent. A consumer **MUST NOT** rely on it.

**A consumer that keeps polling is never refused.** A page that reaches the head of the feed returns a cursor at the head, even when it carries no entries, so a regularly polled cursor stays current however quiet the subscription is.
...
Retention runs from the **end of the covered period** of a **Usage Record**, not from the instant it was accepted: a record is retained for the policy duration measured against the consumption it reports. That is why the two terms are summed rather than alternatives, and the sum yields the invariant the floor exists to deliver:
...
The floor is stated as a formula rather than a constant, so widening either term raises it: a deployment that widens the backfill window **MUST** revalidate retention against the new floor before the wider window takes effect. The same clock bounds how long a **Usage Record**'s dedup identity stays visible to further submissions (`cpt-cf-usage-collector-fr-idempotency`), and the floor's replay-horizon term is what holds the backfill window *strictly* inside retention rather than merely within it. The floor admits no exemption: it binds every GTS type, whatever consumes it.

This is an **operator and plugin deployment obligation**, not gear-level enforcement: retention, archival, and purging remain delegated to the active storage plugin's deployment profile and the platform governance layer. A deployment below the floor for any GTS type is a readiness failure surfaced at operator onboarding and storage-plugin readiness review.
...
The system **MUST** provide a dedicated bulk-import path for historical **Usage Records**, with a bounded backfill window (configurable; default 90 days) enforced as a hard bound, rejecting any submission whose covered period ends further back with an actionable error naming the bound — a period ending exactly at the bound is admitted, since the bound refuses only what lies past it — an origin marker identifying every backfilled **Usage Record**, and every invalidation entry accepted on that path, both as persisted and on every read path, workload isolation from live ingestion such that backfill load does not breach live-path SLOs, and validation identical to the live path but for the covered-period bounds it replaces.

The path **MUST** be reachable on the REST API and on the in-process SDK trait (`cpt-cf-usage-collector-interface-sdk-client`), since it is the only route admitting a covered period older than the live past tolerance and both surfaces must emit and withdraw over that period. It accepts **Usage Records** and invalidation entries alike, and marks both. A deployment **MUST NOT** admit a backfill window that leaves any GTS type's retention below `window + operational replay horizon` (`cpt-cf-usage-collector-fr-billing-retention-floor`), since a **Usage Record** imported at the far edge of a window merely *within* retention would be eligible for purge on arrival. That floor binds every meter, so widening the window raises it for all of them, and retention **MUST** be revalidated against the new floor before the wider window takes effect.

**The window is a hard bound.** The system **MUST NOT** admit a covered period older than it on any surface, and v1 exposes no override reaching past it ([§4.2](#42-out-of-scope)). Two properties rest on that: an aggregate over a period entirely older than the window cannot change ([§5.5](#55-usage-query--aggregation)), and — since every meter retains for at least the window plus one replay horizon, holding the window strictly inside retention — every submission the gear admits falls inside its own idempotency horizon (`cpt-cf-usage-collector-fr-idempotency`). An operator whose history predates the window widens the window before importing, and raises retention with it: the floor is the window plus one replay horizon, so a wider window raises the floor and retention **MUST** be revalidated against it (`cpt-cf-usage-collector-fr-billing-retention-floor`).
...
The system **MUST** expose, via API, per-scope ingestion metadata at minimum at the (tenant, GTS type) granularity: accepted **Usage Record** counts, a quantity summary appropriate to the declared fold — the accrued sum for a `SUM` GTS type, and the observation count together with the latest observation otherwise, since quantities under any other fold are not summable (`cpt-cf-usage-collector-fr-aggregation-fold`) — the latest acceptance-instant watermark and the latest covered period end. Accepted **Usage Record** counts are reported for every GTS type irrespective of its declared fold: they count ingestion activity rather than aggregating the meter, so they are unaffected by the single-fold rule on the aggregate path (`cpt-cf-usage-collector-fr-query-aggregation`). The metadata **MUST** be sufficient for an external reconciliation job to compare gear-side accepted totals against a consumer's processed totals for a time range without a full raw scan, and to detect an emitter that has silently stopped. The counts, the quantity summary, and both watermarks **MUST** reflect one **Usage Record** per dedup identity, as the read paths do, so a submission the storage plugin discards contributes to none of them (`cpt-cf-usage-collector-fr-idempotency`).
...
- **Depends on**: `cpt-cf-usage-collector-fr-ingestion`
- **Rationale**: Revenue assurance compares emitter, gear, and consumer totals; without cheap per-scope counters that comparison is a full scan, and a meter that stops emitting is invisible until an invoice is wrong. Comparing the gear's acceptance-instant watermark against the acceptance instants a consumer has processed lets a reconciliation job distinguish a stalled emitter from a stalled consumer. Detection stays with the consumer because the gear cannot make the judgment correctly: a watermark going quiet means either that an emitter broke or that consumption legitimately ceased — a deleted resource, a workload scaled to zero, a tenant that simply stopped — and only the consumer knows which applies to that resource. A gear-side flag would fire on every legitimate stop. Owning it would also oblige the gear to sweep an unbounded set of (tenant, GTS type) scopes on a timer and to hold a missed-interval threshold that is per-meter policy, for a signal it could not attribute.
- **Actors**: `cpt-cf-usage-collector-actor-platform-operator`, `cpt-cf-usage-collector-actor-usage-consumer`

- [ ] `p3` - **ID**: `cpt-cf-usage-collector-fr-reconciliation-caller-scopes`

The system **MUST** expose the same reconciliation metadata at the (calling gear) and (calling gear, tenant) granularities.

This is deferred beyond v1. The platform keeps the two identities on separate planes. The platform plane carries the calling-gear name, and never carries a tenant. Usage ingestion runs on the tenant plane, whose security context names the caller by opaque subject identifier only. Until one plane carries both, the gear cannot group accepted counts by calling gear. Quotas are unaffected: the gear applies them live, per call, and persists nothing (`cpt-cf-usage-collector-fr-rate-limiting`).

- **Depends on**: `cpt-cf-usage-collector-fr-reconciliation-metadata`
- **Rationale**: Revenue assurance compares what an emitter believes it sent against what the gear accepted from it. The (tenant, GTS type) scope answers this only where one gear owns the meter type.
- **Actors**: `cpt-cf-usage-collector-actor-platform-operator`, `cpt-cf-usage-collector-actor-usage-consumer`
...
The usage feed (`cpt-cf-usage-collector-fr-billing-usage-feed`) **MUST** meet a **recovery objective**: a consumer that has fallen behind **MUST** be able to reach the head of the feed within a bounded recovery time, without breaching the ingestion SLOs (`cpt-cf-usage-collector-nfr-ingestion-latency`, via `cpt-cf-usage-collector-nfr-workload-isolation`). Launch objective: **24 hours behind → caught up within 6 hours.**
...
- Usage feed — per-GTS-type subscription, cursor-paginated, snapshot-consistent, replay-safe reads — `cpt-cf-usage-collector-fr-billing-usage-feed`, `cpt-cf-usage-collector-fr-billing-fields-on-read`
...
**Consumed / Provided Data**: consumes GTS type references; receives the corresponding declarations. The Usage Collector mints, amends, and withdraws no declaration, and sends no usage data to the registry. It makes one temporary write: to recover a declaration the registry lost, it registers that declaration again.
...
- An exact-equality re-submission under an already-accepted idempotency key is silently deduplicated (no duplicate **Usage Record**); a same-key submission whose content differs from a converged **Usage Record** is rejected with an actionable conflict error rather than silently dropped (cross-reference `cpt-cf-usage-collector-fr-idempotency`)
...
5. The aggregated `SUM` for the affected scope and range is reduced accordingly, and the entry appears on the feed at its own feed position.
...
2. Caller constructs an invalidation entry marked as such, referring to that identifier and copying the target's caller-supplied fields — attribution, covered period, quantity, and metadata — plus a reason code and the platform-resolved security context. It supplies no idempotency key: the system derives one from the target.
...
4. System authorizes the emission via PDP and validates the invalidation: the target exists and has converged, is not itself an invalidation, is matched field for field by the copy, and carries a reason code; the system derives the entry's idempotency key from the target; the copied period is bounded by the path, as a **Usage Record**'s period is.
...
- Both entries appear in the usage feed at their own feed positions, so a consumer that already rated the target learns of the withdrawal from the later entry, which names the target it withdraws.

**Alternative Flows**:

- **Already-invalidated target rejected**: an invalidation entry with a different reason code already refers to the target. The derived key gives both one dedup identity, and ingestion rejects the submission with an actionable error stating that the target is already invalidated, since a **Usage Record** is withdrawn at most once.
- **Repeated withdrawal absorbed**: a re-submission of the invalidation with the same reason code — a retry, or a second caller withdrawing the same target for the same reason — is silently deduplicated and returns the accepted invalidation (cross-reference `cpt-cf-usage-collector-fr-idempotency`). An emitter re-running a withdrawal therefore keeps its reason code unchanged, since a changed one is rejected as below.
- **Target not yet converged**: the target has not converged, or the lookup cannot yet rule out an acknowledged target; ingestion rejects the invalidation with a retryable error, which clears within the storage plugin's convergence bound plus its query-path lag bound, and the caller resubmits.
- **Withdrawing a discarded submission**: the emitter copies a submission of its own that the storage plugin discarded under the `eventual` dedup level. The copy does not match the survivor, and ingestion rejects it naming the field that differs; the emitter copies the stored **Usage Record** instead.
- **Key supplied**: the invalidation carries an idempotency key of its own; ingestion rejects it with an actionable error, since the system derives that key from the target.
- **Concurrent withdrawals**: two invalidations of one target with different reason codes are both answered, in separate requests, before their identity has converged. They resolve at the storage plugin's declared dedup level: one accepted and one rejected under `linearizable`, or both acknowledged and the first in commit order kept under `eventual`. The target is withdrawn either way.
...
- **Target older than the backfill window**: the backfill path rejects it with an actionable error naming the bound. No surface admits that covered period, so the target can no longer be withdrawn.
...
1. Consumer requests a page of the feed for its declared GTS type subscription, supplying either no cursor (first page) or the cursor returned with the previous page
...
3. System returns a page over a consistent snapshot, excluding GTS types outside the subscription: entries in the feed's deterministic order, each carrying the unstripped field set of `cpt-cf-usage-collector-fr-billing-fields-on-read`, together with a next cursor before which everything is delivered and final
4. Consumer processes the page, deduplicating by **Usage Record** identifier, and persists the cursor
5. Consumer repeats from the persisted cursor; **Usage Records** accepted after the snapshot are delivered on subsequent pages, always ahead of the persisted cursor

**Postconditions**:

- Consumer has applied every feed entry in its subscription up to its persisted cursor; delivery is at-least-once, and application is made effectively-once by deduplicating on **Usage Record** identifier, with correction linkage sufficient to reconstruct correction history
...
- **Late-arriving Usage Record**: A **Usage Record** accepted later than the end of the period it covers is delivered in feed order, not period order; the consumer detects lateness by comparing the acceptance instant against the end of the covered period
...
- **Covered period beyond the configured backfill bound**: System rejects the submission with an actionable error naming the bound; no override is accepted on any surface
...
- [ ] An exact-equality re-submission under the same idempotency key results in a single stored **Usage Record** (silent dedup), while a same-key submission whose content differs from a converged **Usage Record** is rejected with a duplicate-submission conflict signal rather than silently dropped, and so is the later of two such submissions inside one request; for two submissions racing before their identity has converged, a storage plugin declaring `linearizable` absorbs an identical one and rejects a divergent one, and one declaring `eventual` keeps exactly one, the first in commit order, counts a divergent collision, returns two on no read path and in no reconciliation figure, and delivers only the survivor on the feed; a timed-out submission that commits after the identity has converged on a later one leaves it unchanged — verified by race and late-commit tests through several gateway replicas (cross-reference `cpt-cf-usage-collector-fr-idempotency`)
- [ ] **Usage Records** submitted without an idempotency key are rejected with an actionable error, and so are a **Usage Record** whose key begins with the reserved `inv:` prefix and an invalidation entry that supplies a key at all
- [ ] No admissible submission falls outside its idempotency horizon: a covered period old enough for the referenced GTS type's retention to have lapsed is refused on the backfill window bound before deduplication is consulted, verified by a test that walks a submission's covered period from inside the window to beyond it and asserts a bound rejection rather than any deduplication outcome (cross-reference `cpt-cf-usage-collector-fr-idempotency`, `cpt-cf-usage-collector-fr-backfill`)
- [ ] Retention runs from the covered period rather than the acceptance instant, so a backfilled **Usage Record** arrives with its idempotency horizon already partly spent — at the far edge of the configured backfill window it retains one replay horizon rather than the whole floor; a re-run of that import is deduplicated by the gear while the window still admits it, and is refused on the bound once it does not, so no re-run reaches a state in which the gear admits a silent duplicate (cross-reference `cpt-cf-usage-collector-fr-backfill`, `cpt-cf-usage-collector-fr-billing-retention-floor`)
...
- [ ] Aggregation results can be grouped by tenant, subject, and resource
...
- [ ] The reference an invalidation entry carries is validated at ingestion: the referenced **Usage Record** MUST exist and have converged (a target that has not is rejected with a retryable error, and an identifier that never existed is refused within the storage plugin's convergence and query-path lag bounds), MUST NOT itself be an invalidation entry, and MUST share the full attribution identity — tenant, GTS type, resource, and subject, where presence against absence of a subject is a mismatch; any failure rejects the invalidation with an actionable error (cross-reference `cpt-cf-usage-collector-fr-record-invalidation`)
- [ ] An accepted invalidation makes both the withdrawn **Usage Record** and the invalidation entry contribute nothing to any aggregation that selects them, and affects nothing else: both are returned as persisted by raw query and point lookup, both appear on the feed at their own feed positions and are never removed from it, and every invalidation names the record it withdraws on every one of those paths (cross-reference `cpt-cf-usage-collector-fr-record-invalidation`, `cpt-cf-usage-collector-fr-query-raw`, `cpt-cf-usage-collector-fr-billing-fields-on-read`)
- [ ] A **Usage Record** is withdrawn at most once: every invalidation of it derives one idempotency key and so one dedup identity, so a second invalidation carrying a different reason code is rejected at ingestion with an actionable already-invalidated error, while one carrying the same reason code is absorbed as a duplicate rather than accepted as a second invalidation (cross-reference `cpt-cf-usage-collector-fr-record-invalidation`, `cpt-cf-usage-collector-fr-idempotency`)
- [ ] An invalidation entry is a faithful copy of the **Usage Record** it withdraws: tenant, GTS type, resource, subject, covered period (zero-length where the target is a point event), quantity, and metadata all equal the target's, and a submission differing in any of them is rejected with an actionable error naming the field; the only caller-supplied departures are the reference and the reason code, and the gear assigns it an idempotency key derived from the reference, an identifier, an acceptance instant, and an origin marker in its own right (cross-reference `cpt-cf-usage-collector-fr-record-invalidation`, `cpt-cf-usage-collector-fr-record-identity`, `cpt-cf-usage-collector-fr-record-quantity`, `cpt-cf-usage-collector-fr-record-metadata`, `cpt-cf-usage-collector-fr-usage-windows`)
...
- [ ] An aggregate over a period entirely older than the configured backfill window cannot be changed by any invalidation, since no ingestion path admits an entry whose covered period ends that far back and v1 exposes no override reaching past that bound (cross-reference `cpt-cf-usage-collector-fr-query-aggregation`, `cpt-cf-usage-collector-fr-record-invalidation`, `cpt-cf-usage-collector-fr-backfill`)
...
- [ ] Every invalidation entry carries an idempotency key the system derives as `inv:` followed by the target's identifier, never one the caller supplies: a re-submission with the same reason code is silently deduplicated, and one with a different reason code is rejected as an already-invalidated target (cross-reference `cpt-cf-usage-collector-fr-record-invalidation`, `cpt-cf-usage-collector-fr-idempotency`)
...
- [ ] `LATEST` is determinate where two **Usage Records** of one meter share a covered period: it reports the one with the greatest covered-period end, ties broken by the later acceptance instant and then by the greater identifier in byte order, so repeated queries over the same range return the same quantity, including where the aggregation group spans tenants; verifiable by accepting two records with an identical period and differing quantities and confirming the later-accepted one is reported (cross-reference `cpt-cf-usage-collector-fr-aggregation-fold`)
...
- [ ] On the live ingestion path **every** entry — an ordinary **Usage Record** and an invalidation entry alike — whose covered period **ends** further into the past than the configured past tolerance (default 48 hours) is rejected with an actionable error naming the offending instant, the bound, and the dedicated backfill route, while the backfill path still admits that period within its own window (cross-reference `cpt-cf-usage-collector-fr-live-future-time-bound`, `cpt-cf-usage-collector-fr-backfill`)
...
- [ ] A meter whose consumption must be charged is declared `SUM`; a level-valued meter is pre-integrated at the emitter into an accrued unit rather than charged from `MAX`, `MIN`, or `LATEST`, and a declared nominal sampling interval is served by `types-registry` and never acted on by the gear (cross-reference `cpt-cf-usage-collector-fr-aggregation-fold`, `cpt-cf-usage-collector-fr-quantity-semantics`)
...
- [ ] The feed is deterministic and replay-safe: **Usage Records** are delivered in a deterministic order chosen by the storage plugin, with no ordering promised beyond an invalidation following the entry it withdraws; no entry becomes visible behind a cursor already returned, under any number of concurrent writers, and every page of a live read returns a next cursor; a paginated scan over a consistent snapshot never observes **Usage Records** appearing, disappearing, or mutating mid-scan other than append-only arrivals ahead of the cursor; corrections enter the feed as appended invalidation entries at their own feed positions, after the entries they withdraw, with no feed entry representing a change to an already-delivered **Usage Record** and no accepted invalidation removing either the withdrawn **Usage Record** or the invalidation itself from the feed; and correction linkage is present so a reader can reconstruct correction history (cross-reference `cpt-cf-usage-collector-fr-billing-usage-feed`)
- [ ] A consumer can declare the set of GTS types it consumes, and the feed excludes every other GTS type from its pages and cursor (cross-reference `cpt-cf-usage-collector-fr-billing-usage-feed`)
...
- [ ] Reconciliation metadata is exposed via API at (tenant, GTS type) granularity — accepted **Usage Record** counts for every GTS type irrespective of its declared fold, a fold-appropriate quantity summary (the accrued sum under `SUM`; observation count and latest observation otherwise), the latest acceptance-instant watermark and the latest covered period end — and is sufficient to compare gear-side totals against a consumer's processed totals for a time range without a full raw scan (cross-reference `cpt-cf-usage-collector-fr-reconciliation-metadata`)
- [ ] Reconciliation metadata is exposed at (calling gear) and (calling gear, tenant) granularity — deferred beyond v1, and blocked until the tenant plane carries a calling-gear identity (cross-reference `cpt-cf-usage-collector-fr-reconciliation-caller-scopes`)
...
- [ ] The feed meets its recovery objective: a consumer 24 hours behind reaches the head of the feed within 6 hours while **Usage Records** continue to arrive, with ingestion p95 latency remaining within `cpt-cf-usage-collector-nfr-ingestion-latency` throughout and the §9.0 tolerance applied; the test is run against the subscription under test and the observed read rate is at least five times the subscribed arrival rate (cross-reference `cpt-cf-usage-collector-nfr-replay-throughput`, `cpt-cf-usage-collector-nfr-workload-isolation`)
...
| The platform will carry a calling-gear identity on the tenant plane, so the gear can attribute an accepted entry to the gear that sent it. Today that name lives on the platform plane only, which never carries a tenant | Platform Identity / Platform Security | Verified when the tenant security context carries a calling-gear identity; `cpt-cf-usage-collector-fr-reconciliation-caller-scopes` stays deferred until then |
...
| A charging consumer deduplicates on **Usage Record** identifier across feed delivery, which is at-least-once: an overlapping replay, a resumed cursor, and a redelivered page each present an identifier the consumer may already have rated (`cpt-cf-usage-collector-fr-billing-usage-feed`). Ingestion contributes no such case, since the backfill window bound keeps every admissible submission inside its idempotency horizon (`cpt-cf-usage-collector-fr-idempotency`), so the consumer's dedup state need span only its own replay reach | BSS Rating owner / Usage Collector Maintainers | Verified against the Rating gear's deduplication design, and exercised by an overlapping-replay test before the release candidate |
...
- **Import and correction beyond the backfill window.** The configured window is a hard bound in v1 ([§4.2](#42-out-of-scope)), so usage older than it can be neither imported nor withdrawn. Earlier revisions of this document reserved an elevated-authorization override reaching past it; that reservation is withdrawn. It was never given a permission, an error, or a bound of its own, and it was the only construct able to place a submission outside its own idempotency horizon — which is a contract question rather than an authorization one, and is not answered by granting a role. Reintroducing it means designing all three and settling whether re-importing already-charged history is legitimate at all. The trigger for revisiting it is a migration or a dispute that genuinely needs to place usage older than the window, and cannot be served by widening the window ahead of the import.
