...
- An entry whose period ends exactly on the upper bound belongs to the **next**
  ...
  July query, not a June one. Edge-to-edge tiling stays the expected emission
  shape, and the consumer shifts its range forward by one period instead.
- The width of the period does not matter. A one-month entry is selected by the
  one range that holds its `window_end`. A consumer that queries finer than the
  emission grain gets the whole entry in that range, and nothing in the rest.
...
- An SPI contract test for the rule: a point event on the lower bound is selected,
  ...
  the range is selected when the range holds its `window_end`.
...
- Bad, because a query finer than the emission grain sees a coarse entry in one
  range only, and nothing in the others.
