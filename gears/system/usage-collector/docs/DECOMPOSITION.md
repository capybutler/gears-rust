...
- `usage-collector` (host gear crate):
  ...
  - `toolkit-odata` — `Page<T>` re-export and OData query parsing (`$filter`, `$orderby`, `limit`, `cursor`), plus `toolkit_odata::validate_cursor_against` for cursor binding checks.
